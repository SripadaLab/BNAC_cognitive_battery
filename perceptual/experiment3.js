...
function jglMakeGrating(width, height, sf, angle, phase, pixPerDeg) {
	...
	//provide sf in cycles per pixel, and provide pixPerDeg to convert
	sf = sf * pixPerDeg;
	...
	x = jglMakeArray(-Math.PI, xStep, Math.PI); // to nudge jglMakeArray to include +PI // stupid
	...
	xTilt = Math.cos(angleInRad) * sf * (height/pixPerDeg); //2 * Math.PI;
	yTilt = Math.sin(angleInRad) * sf * (width/pixPerDeg); //2 * Math.PI;
...
function Canvas(id, back_id, height, width) {
...
function makeStim(canvas, backcanvas, angle, contrast, sf, sigma, ppd, size) {
	var jgl_canvas = new Canvas(canvas, backcanvas, size, size)
	var arr = jglMakeGrating(size, size, sf, angle, 0, ppd)
